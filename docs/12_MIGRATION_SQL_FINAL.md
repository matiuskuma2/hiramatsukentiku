# 最終マイグレーション SQL（v4 準拠）

> **目的**: 01_DB_SCHEMA_DESIGN_v4.md の全25テーブルを一括作成するマイグレーションファイル。
> **適用先**: `migrations/0001_initial_schema.sql`
> **対応DB**: Cloudflare D1 (SQLite)
> **CHECK制約**: 11_ENUM_STATUS_SPEC.md に基づく全enum値を制約
...
以下を `migrations/0001_initial_schema.sql` として保存する。
...
| INSERT (初期データ) | 9行 (system_settings) |
| CHECK 制約 | 22 |
...
-- system_settings 初期データ確認（9件）
SELECT COUNT(*) AS settings_count FROM system_settings;

-- CHECK制約の検証（不正値を INSERT して失敗することを確認）
-- 例: 不正な status
-- INSERT INTO projects (project_code, project_name, lineup, status)
-- VALUES ('TEST-001', 'テスト', 'SHIN', 'invalid_status');
-- → CHECK constraint failed: status IN (...) エラーが発生すること
```

---

## マイグレーション分割戦略（D1 バッチ制限対応）

D1 のバッチサイズ制限（100文）に対応するため、マイグレーション適用時は以下に注意:

1. **`wrangler d1 migrations apply`** は自動的にファイル内の SQL 文を分割して実行する
2. 上記の単一ファイルで問題なく適用可能（全23 CREATE TABLE + 47 CREATE INDEX + 9 INSERT = 79文 < 100文制限）
3. 将来のマイグレーション追加は `0002_add_xxx.sql` 等の連番ファイルで管理

---

*最終更新: 2026-03-07*
*対応ドキュメント: 01_DB_SCHEMA_DESIGN_v4.md, 11_ENUM_STATUS_SPEC.md*
